...
from PyQt5.QtWidgets import (QApplication, QMainWindow, QWidget, QStackedWidget,
    QHBoxLayout, QVBoxLayout, QPushButton, QTextEdit, QLineEdit, QListWidget,
    QListWidgetItem, QLabel, QFileDialog, QMessageBox, QMenuBar, QMenu, QCheckBox,
    QScrollArea, QInputDialog, QAbstractItemView)
...
class NewNoteWidget(QWidget):
    ...
    def init_ui(self):
        ...
        self.url_label.hide(); self.btn_copy_url.hide()
        ...
        hl = QHBoxLayout(); hl.addWidget(self.btn_add_images); hl.addWidget(self.btn_share_note)
        l.addLayout(hl)
        ul = QHBoxLayout(); ul.addWidget(QLabel("Your Note URL:")); ul.addWidget(self.url_label); ul.addWidget(self.btn_copy_url)
        l.addLayout(ul)
    ...
    def add_images(self):
        ...
        if files:
            for f in files:
                if len(self.selected_images) >= 5: break
    ...
    def share_note(self):
        ...
        if not self.selected_images:
            ...
            for ip in self.selected_images:
                try:
                    ...
                    if not mm: mm = "application/octet-stream"
        ...
        if r.status_code == 200:
            ...
            if js.get("success"):
                ...
                self.note_url = link
                QMessageBox.information(self, "Success", f"Note shared!\nLink: {link}")
                self.url_label.setText(link)
                self.url_label.show(); self.btn_copy_url.show()
                ...
                QMessageBox.warning(self, "API Error", f"Could not share note: {er}")
        else:
            QMessageBox.warning(self, "HTTP Error", f"Status: {r.status_code}\n{r.text}")
    def copy_url(self):
        if self.note_url:
...
class NoteDetailWidget(QWidget):
    def __init__(self, note, back_callback):
        ...
        l = QVBoxLayout(self)
        l.addWidget(QLabel(f"Title: {self.note.get('title', 'Untitled')}"))
        c = QLabel(self.note.get("note_text", ""))
        c.setWordWrap(True)
        s = QScrollArea(); s.setWidgetResizable(True)
        cw = QWidget(); cl = QVBoxLayout(cw); cl.addWidget(c); s.setWidget(cw)
        l.addWidget(QLabel("Content:")); l.addWidget(s)
        ...
        if im:
            ...
            for i in im:
                ...
                if not pm.isNull():
                    ...
                    lb = QLabel(); lb.setPixmap(th); hl.addWidget(lb)
            l.addWidget(QLabel("Images:")); l.addLayout(hl)
        bdel = QPushButton("Delete Note"); bback = QPushButton("Back")
        l.addWidget(bdel); l.addWidget(bback)
...
class MyNotesWidget(QWidget):
    def __init__(self, open_note_callback):
        ...
        l.addLayout(left_panel, 1); l.addLayout(right_panel, 2)
    ...
    def refresh_data(self):
        self.load_categories(); self.load_notes_for_category()
        ...
        for n in reversed(d["notes"]):
            if cid is None:
                if n["category_id"] is None:
                    it = QListWidgetItem(f'{n["title"]} - {n["timestamp"]}')
...
class MainWindow(QMainWindow):
    ...
    def change_theme(self, th):
        ...
        if th=="dark": self.apply_dark_palette()
        else: self.apply_light_palette()
